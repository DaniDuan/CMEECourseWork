...
"""Script that runs a discrete-time version of the Lotka-Volterra model with Gaussian fluctuation
    of resource growth rate (r)"""
...
def run_LV(R=10, C=3, r=1., a=0.1, z=1.5, e=0.75, K=100, generations=30):
    """Runs discrete-time Lotka-Volterra model 'generations' times, with Gaussian fluctuation
        of resource growth rate (r)"""
    ...
    for i in range(generations):
        ...
        R = pops[i, 0]
        C = pops[i, 1]
        epsilon = np.random.normal(loc=0, scale=0.2)  # loc = mean, scale = SD
...
def main(argv):
    ...
    f1.savefig("../results/LV4_timeseries.pdf")
    ...
    #p.show()
    f2.savefig("../results/LV4_pop_comparison.pdf")
